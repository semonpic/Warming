...
namespace R100Test
{
    class CHostDBCtrl
    {
        ...
        //*****************************************用戶資料寫入資料庫*********************************************************//
        public int InsertUserInfo(string strUserID, int nRIrisQuality, int nLIrisQuality, string strFacePath, string strRPath, string strLPath, string strInsertDate, string strUpdateDate,string strUserName,int Level)
        {
            if(m_Conn == null)
            ...
            string strSqlAddUserName = "UPDATE EnrolledUserInfo Set Level ='" +Level.ToString() +"' ,EXT_NAME='" + strUserName + "' WHERE USER_ID='" + strUserID + "'";
            if (m_Conn.State == ConnectionState.Closed)
            {
                m_Conn.Open();
            }
            ...
            command.ExecuteNonQuery();
            command = new OleDbCommand(strSqlAddUserName, m_Conn);
            command.ExecuteNonQuery();
            string t = m_ConnStr;
